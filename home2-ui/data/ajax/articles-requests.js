...
export async function load(page, pageSize, publishedOnly = false) {
    console.debug(`Articles (page=${page}, pageSize=${pageSize}, published=${publishedOnly}) loading`);
    let response = await (await superagent()).get(`/blog/articles?page=${page}&pageSize=${pageSize}&published=${publishedOnly}`)
        .use(api);
...
export async function loadOne(articleId) {
    console.debug(`Article ${articleId} loading`);
    let response = await (await superagent()).get(`/blog/articles/${articleId}`)
        .use(api);
...
export async function update(id, update) {
    ...
    let response = await (await superagent()).put(`/blog/articles/${id}`, update)
        .use(api);
...
export async function remove(id) {
    ...
    let response = await (await superagent()).delete(`/blog/articles/${id}`)
        .use(api);
...
const ArticlesRequests = {load, loadOne, update, remove}
