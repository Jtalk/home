  ...
  return (
    <div>
      <Form error={loading === Loading.ERROR || updating === Updating.ERROR} success={updating === Updating.UPDATED}>
        ...
        <Form.Input
          label="New Logo Name"
          placeholder="A name to show in the <alt/> tag"
        ...
        />
        <Form.Input
          label="New Logo target"
          placeholder="Target URL"
          value={newLogoForm.data.href || ""}
          onChange={newLogoForm.updater.change("href")}
        />
        <ErrorMessage message={errorMessage} />
        <Form.Button primary onClick={newLogoForm.onSubmit(createNewLogo)}>
          Create
        </Form.Button>
      </Form>
      <Divider />
      <List divided relaxed>
        {footer.logos.map((logo, i, logos) => {
          return (
            <List.Item key={logo.name}>
              <List.Content floated="right">
                <LazyIcon
                  link={i === 0}
                  name={i === 0 ? "lock" : "up arrow"}
                  onClick={updater.reorder(i, i - 1, "logos")}
                />
                <LazyIcon
                  link={i === logos.length - 1}
                  name={i === logos.length - 1 ? "lock" : "down arrow"}
                  onClick={updater.reorder(i, i + 1, "logos")}
                />
                <LazyIcon link color="red" name="remove" onClick={updater.removeItem(i, "logos")} />
              </List.Content>
              <List.Content>
                <List.Header as="h3">{logo.name}</List.Header>
                <List.Description>
  ...
  return (
    <div>
      ...
      <Form error={updating === Updating.ERROR || loading === Loading.ERROR} success={updating === Updating.UPDATED}>
        <h2>Edit links</h2>
        <Form.Input
          label="New Link Caption"
          placeholder="A text to show for this link"
        ...
        />
        <ErrorMessage message={errorMessage} />
        <Form.Button primary onClick={newLinkForm.onSubmit(createNewLink)}>
          Create
        </Form.Button>
      </Form>
      <Divider />
          ...
          return (
            <List.Item key={link.caption}>
              <List.Content floated="right">
                ...
                />
                <LazyIcon
                  link={i === links.length - 1}
                  name={i === links.length - 1 ? "lock" : "down arrow"}
                  onClick={updater.reorder(i, i + 1, "links")}
                />
                <LazyIcon link color="red" name="remove" onClick={updater.removeItem(i, "links")} />
              </List.Content>
              <List.Content>
                <List.Header as="h3">{link.caption}</List.Header>
                <List.Description>{link.href}</List.Description>
              </List.Content>
            </List.Item>
          );
        })}
