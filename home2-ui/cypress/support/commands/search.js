...
Cypress.Commands.add("containsSearchResult", (index, href, header, description) => {
  cy.get("div[data-id=header-search-bar]").within(() => {
    cy.get(".results a[data-id=search-result-link]").eq(index).should("have.attr", "href", href);
    cy.get("a[data-id=search-result-link] div[data-id=search-result-title]").eq(index).should("have.text", header);
    cy.get("a[data-id=search-result-link] div[data-id=search-result-description]")
      .eq(index)
