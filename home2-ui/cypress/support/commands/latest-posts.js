...
Cypress.Commands.add("containsLatestPosts", (num) => {
  ...
  for (let i = 0; i < num; i++) {
    cy.get("div[data-id=latest-posts] div[data-id=latest-post] h4[data-id=latest-post-caption] a")
      .eq(i)
      .should("have.attr", "href", `/blog/articles/stub-post-${i + 1}`)
      .should("have.text", `Stubby's Post #${i + 1}`);
    cy.get("div[data-id=latest-posts] div[data-id=latest-post] div[data-id=latest-post-created-timestamp]")
      .eq(i)
