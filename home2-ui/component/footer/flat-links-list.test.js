...
describe("<FlatLinksList/>", () => {
  ...
  it("renders multiple links with separators", () => {
    ...
    expect(result.find("div").childAt(0).filter("a").props()).toMatchObject({
      href: links[0].href,
      children: links[0].caption,
    });
    expect(result.find("div").childAt(1).props()).toMatchObject({ separator: separator });
    expect(result.find("div").childAt(2).filter("a").props()).toMatchObject({
      href: links[1].href,
      children: links[1].caption,
    });
    expect(result.find("div").childAt(3).props()).toMatchObject({ separator: separator });
    expect(result.find("div").childAt(4).filter("a").props()).toMatchObject({
      href: links[2].href,
      children: links[2].caption,
    });
