...
export function useForm({ init, autoSubmit, secure } = {}) {
  ...
  let updater = useMemo(() => new Updater(data, updateData, setData, autoSubmitter), [
    data,
    updateData,
    setData,
    autoSubmitter,
...
class Updater {
  ...
  change = (...path) => {
    return (e, { value }) => {
      let oldValue = get(this.data, path);
      if (isEqual(oldValue, value)) {
        console.debug(
          `Not changing value at path:`,
          path,
          showSecurely(oldValue, this.secure),
          showSecurely(value, this.secure)
        );
      } else {
        console.debug(
          `Changing value at path:`,
          path,
          showSecurely(oldValue, this.secure),
          showSecurely(value, this.secure)
        );
  ...
  reorder = (fromIndex, toIndex, ...path) => {
    return (e) => {
      let collection = get(this.data, path) || [];
